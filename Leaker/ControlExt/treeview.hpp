#ifndef LEAKER_TREEVIEW_H_
#define LEAKER_TREEVIEW_H_
...
public:
    ...
    CTreeViewUI() : _root(NULL), m_dwDelayDeltaY(0), m_dwDelayNum(0), m_dwDelayLeft(0)
    {
    ...
    Node* AddNode(LPCTSTR text, Node* parent = NULL)
    {
		...
		pListElement->SetAttribute(_T("itemtextcolor"),_T("#FFFFFFFF"));
        Node* node = new Node;
        ...
        if( node->data()._level < 3 ) {
            if( node->data()._child_visible ) html_text += _T("<a><i treeview_expand.png 2 1></a>");
            else html_text += _T("<a><i treeview_expand.png 2 0></a>");
        }
    ...
    void SetChildVisible(Node* node, bool visible)
    {
        ...
        for( int i = begin->data()._pListElement->GetIndex(); i <= end->data()._pListElement->GetIndex(); ++i )
        {
            ...
            if( _tcscmp(control->GetClass(), _T("ListLabelElementUI")) == 0 )
            {
                ...
                {
                    Node* local_parent = ((CTreeViewUI::Node*)control->GetTag())->parent();
